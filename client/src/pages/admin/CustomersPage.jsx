    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#a3948b] mt-1">Create, edit, or remove customer accounts</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 bg-white border border-[#e5ddd5] rounded-xl text-sm w-full md:w-80 focus:ring-1 focus:ring-[#c2b2a6] outline-none transition-all shadow-sm"
            />
          </div>
          <button
            type="button"
            onClick={openCreateCustomer}
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-[#3b302a] text-white text-sm font-semibold hover:bg-[#2e2622] shadow-sm"
          >
            <Plus size={17} />
            Add Customer
          </button>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[#fcfaf7] border-b border-[#e5ddd5]">
                ...
                <th className="px-6 py-4 text-xs font-semibold text-[#a3948b] uppercase tracking-wider">Default Address</th>
                <th className="px-6 py-4 text-xs font-semibold text-[#a3948b] uppercase tracking-wider">Joined</th>
            ...
            <tbody className="divide-y divide-[#f3ede8]">
              {customersLoading && (
                <tr>
                  <td colSpan="5" className="px-6 py-8 text-center text-[#6b5e55]">
              ...
              {customersError && !customersLoading && (
                <tr>
                  <td colSpan="5" className="px-6 py-8 text-center">
                    <p className="text-sm text-rose-600">{customersError}</p>
                    <button
                      type="button"
                      onClick={refreshCustomers}
                      className="mt-3 px-4 py-2 rounded-xl border border-[#d8ccc2] text-sm text-[#5f5149] hover:bg-[#f8f5f2]"
                    >
                      Try Again
                    </button>
                  </td>
              ...
              {!customersLoading && !customersError && filteredCustomers.length === 0 && (
                <tr>
                  <td colSpan="5" className="px-6 py-8 text-center text-sm text-[#8c7d73]">
                    No customers found.
                  </td>
                </tr>
              )}
                ...
                <tr key={customer.id} className="hover:bg-[#fcfaf7] transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-[#3b302a]">{customer.name}</p>
                        <p className="text-xs text-[#a3948b]">ID: {customer.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <span className="flex items-center gap-2 text-sm text-[#6b5e55]">
                  ...
                  <td className="px-6 py-4 max-w-xs">
                    <p className="text-sm text-[#6b5e55] truncate">
                      {customer.defaultAddress
                        ? `${customer.defaultAddress.addressLine1}, ${customer.defaultAddress.city}`
                        : "No saved address"}
                    </p>
                  </td>
                  <td className="px-6 py-4 text-sm text-[#6b5e55]">{customer.joinedDate}</td>
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-2">
                      <button
                        type="button"
                        onClick={() => openEditCustomer(customer)}
                        className="p-2 text-[#8c7d73] hover:text-[#3b302a] hover:bg-[#f3ede8] rounded-xl transition-all"
        ...
        {drawerOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 bg-[#3b302a]/25 backdrop-blur-sm z-[60]"
              onClick={closeDrawer}
            />
            <motion.aside
              initial={{ x: "100%" }}
              ...
              <form onSubmit={handleSave} className="p-8 space-y-8">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-[#a3948b] mt-1">
                ...
                <div className="flex items-center gap-5 p-6 bg-[#fcfaf7] rounded-3xl border border-[#e5ddd5]">
                  ...
                  <div>
                    <h4 className="text-xl font-semibold text-[#3b302a]">{selectedCustomer?.name || "New Customer"}</h4>
                    <p className="text-[#a3948b]">{selectedCustomer?.email || "Created by admin"}</p>
                    <p className="text-xs text-[#8c7d73] mt-2">
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-[#a3948b] uppercase tracking-widest border-b border-[#e5ddd5] pb-2">
                    Default Address
                  </h4>
                ...
                {!creatingCustomer && (
                  <div className="space-y-3">
                    <h4 className="text-xs font-bold text-[#a3948b] uppercase tracking-widest">Saved Addresses</h4>
                    {selectedCustomer.addresses.length === 0 ? (
                      <p className="text-sm text-[#8c7d73] bg-[#fcfaf7] border border-[#e5ddd5] rounded-2xl px-4 py-3">
                        No saved addresses.
                      </p>
                    ) : (
                      selectedCustomer.addresses.map((address, index) => (
                        <div key={`${address.addressLine1}-${index}`} className="rounded-2xl border border-[#e5ddd5] bg-white px-4 py-3">
                          <p className="flex items-center gap-2 text-sm font-semibold text-[#3b302a]">
                            <MapPin size={15} />
                            {address.fullName || selectedCustomer.name}
                          </p>
                          <p className="mt-1 text-sm text-[#6b5e55]">
                            {[address.addressLine1, address.addressLine2, address.city, address.district, address.postalCode, address.country]
                              .filter(Boolean)
                              .join(", ")}
                          </p>
                        </div>
                ...
                {formError && (
                  <p className="text-sm text-rose-600 bg-rose-50 border border-rose-100 rounded-2xl px-4 py-3">
                    {formError}
                  </p>
                )}
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    type="button"
                    onClick={closeDrawer}
                    className="flex-1 py-3 rounded-xl bg-white border border-[#d8ccc2] text-[#5f5149] text-sm font-semibold hover:bg-[#f8f5f2]"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={saving}
                    className="flex-1 inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-[#3b302a] text-white text-sm font-semibold hover:bg-[#2e2622] disabled:opacity-60"
                  >
...
const Field = ({ label, name, type = "text", value, onChange, required = false, placeholder = "" }) => (
  <label className="block">
    <span className="block text-xs font-semibold text-[#a3948b] uppercase tracking-wider mb-2">
      {label}
    </span>
    <input
      name={name}
