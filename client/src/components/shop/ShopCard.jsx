...
export default function ShopCard({ product, onOpen, onQuickAdd }) {
  ...
  const displayImg = activeColor?.images?.[0]?.url || images[0]?.url || SHOP_IMAGE_FALLBACK;
  const displayImg2 = activeColor?.images?.[1]?.url || images[1]?.url || displayImg;

  const badge = product.isFeatured ? "Best Seller" : product.isNewArrival ? "New" : null;
    ...
    <article className="shop-card" onClick={onOpen}>
      ...
      <div className="shop-card-info">
        ...
        <div className="shop-card-swatches">
          {colors.slice(0, 5).map((color) => (
            <button
              ...
              type="button"
              className={`shop-card-swatch ${isColorSoldOut(color) ? "sold-out" : ""} ${
                activeColor === color ? "active" : ""
              }`}
              style={{ backgroundColor: color.colorCode || "#ccc" }}
              title={isColorSoldOut(color) ? `${color.colorName} - Sold Out` : color.colorName}
              onClick={(event) => {
                event.stopPropagation();
        ...
        <p className="shop-card-installment">
          or 3 x LKR {Math.round(Number(product.price || 0) / 3).toLocaleString()} with Koko
        </p>
      </div>
    </article>
  );
